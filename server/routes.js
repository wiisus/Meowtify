...
// Legacy Qobuz fallback base
// Switched to US region as requested
const QOBUZ_API_BASE = "https://us.qobuz.squid.wtf/api";

async function registerRoutes(app) {
  const spotifyClient = config.apiKeys && config.apiKeys.spotify !== "YOUR_SPOTIFY_API_KEY" && config.apiKeys.spotify_secret !== "YOUR_SPOTIFY_CLIENT_SECRET" ? new SpotifyClient(config.apiKeys.spotify, config.apiKeys.spotify_secret) : null;
  ...
  // Shared headers to mimic browser; cookies optional (kept minimal to honor "headers only" intent)
  ...
  // Helper: proxy bytes from a Web Fetch Response to Express res
  ...
  app.get("/api/music/search", async (req, res) => {
    try {
      ...
      // Transform Qobuz response to our schema
      let tracks = [];
      let albums = [];
      let artists = [];
      ...
      if (data.data?.tracks?.items) {
        tracks = data.data.tracks.items.map((track) => ({
          ...
          streamUrl: `${this.backendUrl}/api/music/stream/${track.id}`,
          downloadUrl: `${this.backendUrl}/api/music/download/${track.id}?quality=27`,
  ...
  // Music quick search endpoint - returns top 5 results for typing suggestions
  app.get("/api/music/search/quick", async (req, res) => {
    try {
      ...
      // Extract top 5 tracks and albums only
      let tracks = [];
      let albums = [];
  ...
  app.get("/api/music/track/:trackId", async (req, res) => {
    try {
      ...
      const track = {
        ...
        streamUrl: `${this.backendUrl}/api/music/stream/${data.id}`,
        downloadUrl: `${this.backendUrl}/api/music/download/${data.id}?quality=27`,
  ...
  // Music artist discography endpoint
  ...
  // Music download endpoint — squid-only, enforce FLAC 27 and validate integrity
  app.get("/api/music/download/:trackId", async (req, res) => {
    try {
      ...
      // Enforce 27 for FLAC path; allow 5 only explicitly but we don't auto-downgrade
  ...
  app.get("/api/spotify-token", async (req, res) => {
    try {
      if (!spotifyClient) {
        return res.status(503).json({ message: 'Spotify API not configured.' });
      }
  ...
  app.get("/api/recommendations", async (req, res) => {
    try {
      if (!spotifyClient) {
        return res.status(503).json({ message: 'Spotify API not configured. Please add your API key to meowtify/config.json' });
      }
  ...
  app.get("/api/spotify/search", async (req, res) => {
    try {
      ...
      res.status(500).json({ message: 'Failed to search Spotify track.' });
    }
  });
  ...
  app.get("/api/genre-tracks", async (req, res) => {
    try {
      ...
      // This is a placeholder. You'll need to implement the logic to get tracks for a genre.
      // This might involve searching Spotify for a playlist with that genre name, or using Last.fm's tag.gettoptracks.
      res.json({ tracks: [] });
    } catch (error) {
  ...
  app.get("/api/lyrics", async (req, res) => {
    try {
        ...
        // Using a public lyrics API as a fallback
